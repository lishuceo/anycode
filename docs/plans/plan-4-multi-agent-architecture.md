...
- OpenClaw 多 agent 群聊实现（见 `docs/research/openclaw-analysis.md`）
...
2. ✅ `config/personas/pm.md` — Agent 注册表（chat: Sonnet/readonly, dev: Opus/full）
3. ✅ `src/agent/router.ts` — Binding Router (first-match-wins) + `shouldRespond()` @mention 路由
4. ✅ `config/personas/pm.md` — Chat Agent 只读 system prompt
...
5. ✅ `config/personas/pm.md` — chat: `replyMode: 'direct'`, dev: `replyMode: 'thread'`
6. ✅ `config/personas/pm.md` — 更新为对话风格 + `start_discussion_thread` 使用引导
...
4. `config/personas/pm.md` — 加入 invoke_agent 使用引导
