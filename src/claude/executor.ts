...
### 何时必须重启
- 当**服务部署目录**（\`${process.cwd()}\`）中的代码发生了变更时，必须 build + 重启才能生效。常见场景：在部署目录执行了 \`git pull\`、直接修改了源码文件等
- 如果你在隔离工作区中修改代码并推送到远程，部署目录的代码没有变化，**不需要重启**
- 仅修改了配置文件（如 config/agents.json、.env）不需要重启，热重载会自动生效
...
- 你的工作目录是服务仓库的隔离 clone，修改不会直接影响运行中的实例。在服务部署目录中执行 git pull + build + restart
