 ...
 *   4. 如果 URL 是本地路径 → resolve() 绝对路径
 *   5. 非 git 目录或无 remote → 返回 workDir 的 resolve() 结果
 *
...
function resolveIdentity(workDir: string): string {
  ...
  try {
    remoteUrl = execFileSync('git', ['-C', workDir, 'config', '--get', 'remote.origin.url'], {
      timeout: 5_000,
  ...
  if (!remoteUrl) return resolve(workDir);
  ...
  // Case 3: local path → resolve relative to workDir (not process.cwd)
  return resolve(workDir, remoteUrl);
}
